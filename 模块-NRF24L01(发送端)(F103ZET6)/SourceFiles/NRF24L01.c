...
void NRF24L01_Rx_EXTI_Init(void)
{
	...
	//2.GPIO链接EXTI.
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOG, GPIO_PinSource6);//PG6
	//3.初始化EXTI
	EXTI_InitTypeDef EXTI_InitStruct;
	
	EXTI_InitStruct.EXTI_Line = EXTI_Line6;//通道6
...
void NRF24L01_Rx_Mode_Init(void)
{
	...
	NRF24L01_Write_Reg(NRF24L01_Rx_Pw_P0, 32);//32字节有效
...
void NRF24L01_RxPacket(void)
{
	...
	NRF24L01_Write_Reg(NRF24L01_STATUS, State);
	//读取接收到的数据
	NRF24L01_Read_Buf(NRF24L01_Read_RX, RxBuf, 32);
	//清除接收寄存器
...
{
	//判断什么标志位导致的中断
	if(EXTI_GetITStatus(EXTI_Line6) == SET)//判断是不是通道6导致的中断
	{
		NRF24L01_RxPacket();
		//清除中断服务函数标志位
